...
export default function News() {
  const data = [
    {
      para: "MetaBrix ranked among top 10 award winning Startups in ISF",
    },
    {
      para: "MetaBrix featured in NASSCOM Gen-AI Startup Landscape Report",
    },
    {
      para: "The new age Generative AI startup : MetaBrix Founders Interview",
    },
  ];
  ...
  return (
    <div className="mb-20 py-4 md:px-8 px-4">
      ...
      <div className="grid lg:grid-cols-3 sm:grid-cols-2 gap-4 mt-20 ">
        {data.map((e, i) => {
          return (
            <div key={i} className="rounded-xl p-4 grid place-content-center">
              <div className=" h-fit p-4 overflow-hidden rounded-3xl grid gap-4 place-content-center border-2 w-fit max-w-[400px]">
                <motion.img
                  src={`/assets/images/news${i + 1}.svg`}
                  className=" object-contain object-center h-full"
                />

                <div className="p-1">{e.para}</div>

                <div className=" pt-4">
                  <div className=" font-bold border-2 p-4 text-center rounded-xl">
                    {" "}
                    Read More{" "}
                  </div>
                </div>
